...
Reparameterizing, we can write the probability $p(v_{ud} | \bm X_u, \bm \zeta_d,
 \bm \psi_d)$ of an affirmative vote with the probit or logistic function
...
\emph{popularity} $b_d$. \footnote{Popularity is also called
  \emph{difficulty}, and polarity is called \emph{discrimination}, in the
...
popularities, topics, and coefficients. Computing the posterior exactly
...
``popularity'' term, which acts as an intercept).  Many of these
...
to both the standard ideal point model and the ideal point topic model---which uses variational methods \citep{jordan:1999}.
...
multinomial parameters $\bm \phi_d$ and Dirichlet parameters $\bm \gamma_d$,
...
the data from \url{www.govtrack.us}, an independent Website which provides
...
bill's popularity and polarity parameters. \myfig{topics} shows
...
\begin{figure}
  ...
  \caption{Topics can be visualized in the same latent political space
    ...
    and \emph{clause, motion, chair, ...}) with popularity 4.68 and
...
the $111^{\textrm{th}}$ Congress.  This bill, the \emph{NASA Authorization Act
...
Paul, a Republican in the $111^{\textrm{th}}$ Congress, was also poorly
...
parameterized by regression coefficient $\Lambda \ge 0$, and \verb!IPTM! is parameterized by
topics $K$).
