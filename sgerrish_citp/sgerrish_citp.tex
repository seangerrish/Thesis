...
\vspace{20pt}
  \begin{itemize}
  \item Understanding the main patterns of influence and events in collections of text documents: %    \item How to find influential text documents
    Discovering international relations using news articles
%    \end{itemize}
  \vspace{20pt}
  \item Better understanding government: Discovering how lawmakers feel about issues based on their votes
%    \begin{itemize}
%    \item Understand lawmakers' issue preferences
...
\frame {
  ...
  \item Fit the foreign relations model in an unsupervised way \tiny \textcolor{gray}{(c.f. \cite{chang:2009})} \normalsize
...
%  \center
  Ideal points:
  \begin{align*}
    p(v_{ud} = \mbox{Yes}| \ldots) = \mbox{logistic}(x_u \cdot a_d + b_d) \\
  \end{align*}
  Issue-adjusted ideal points:
  \begin{align*}
    p(v_{ud} = \mbox{Yes}| \ldots) = \mbox{logistic}((x_u \textcolor{blue}{+ \bm z_u^T \theta_d}) \cdot a_d + b_d)
  \end{align*}
  \normalsize
}
...
% %H. Res 806 (amending an education and environment trust fund)
% % \vspace{7pt}
% % & \includegraphics[width=0.4\textwidth]{figs/3397_ideal_point_10.pdf}
% % & \includegraphics[width=0.4\textwidth]{figs/3397_adjusted_ideal_point_10.pdf} \\
\hline
% To restore sums to the Highway Trust Fund, and for other purposes
 \small Providing for conditional adjournment of %and recess of
 Congress
...
% %& \includegraphics[width=0.4\textwidth]{figs/3397_ideal_point_8.pdf}
% %& \includegraphics[width=0.4\textwidth]{figs/3397_adjusted_ideal_point_8.pdf} \\
...
% %& \includegraphics[width=0.4\textwidth]{figs/3397_ideal_point_6.pdf}
% %& \includegraphics[width=0.4\textwidth]{figs/3397_adjusted_ideal_point_6.pdf} \\
\hline
...
% %& \includegraphics[width=0.4\textwidth]{figs/3397_ideal_point_4.pdf}
% %& \includegraphics[width=0.4\textwidth]{figs/3397_adjusted_ideal_point_4.pdf} \\
...
% %& \includegraphics[width=0.4\textwidth]{figs/3397_ideal_point_2.pdf}
% %& \includegraphics[width=0.4\textwidth]{figs/3397_adjusted_ideal_point_2.pdf} \\
\hline
...
% \frame {
%   \frametitle{Procedural Cartels}
%   \emph{Procedural cartel theory} explains the difference in voting patterns between substantive issues and procedural issues.
% \tiny
% %  \begin{center}
% \textcolor{gray}{
%  %   \begin{tabular}{ll}
%   (\cite{fenno:1965}, \cite{jones:1964},
%   \cite{cox:2002}, \cite{cox:1993})
% %  \end{tabular}
%   }
% %  \end{center}
%   \normalsize
...
% \begin{itemize}
%   \item
%     \emph{Parties in the House -- especially the majority party -- are a
%       species of 'legislative cartel' [ which usurp the power ] to make
%       rules governing the structure and process of legislation.}
%       \tiny \textcolor{gray}{\citet{cox:1993}} \normalsize
%   %% The cartel's primary goal in procedural control is to structure the rules
%   %% of the House to ensure party unity and to dissuade defection among
%   %% party members.
%   \item The majority party delegates an agenda-setting monopoly to senior
%   partners in the party. %, who set the procedural agenda in the House
% \item The cartel ensures that senior members hold
%   agenda-setting seats (such as committee chairs) while rank-and-file
%   members of the party support agenda-setting decisions % (instead of
% %  defecting).
%   \tiny \textcolor{gray}{\citep{cox:2005}} \normalsize
%   \end{itemize}
% }
...
\frame {
  \frametitle{Summary}
  A new model for finding patterns in collections of text data
  \begin{itemize}
  % \item A method to find influence in text collections
    \item Map the interactions of countries through time
  \end{itemize}
  \vspace{30pt}
  Automatically infer lawmakers' voting patterns on issues
  \begin{itemize}
    \item Traditional ideal points are limited and hard to interpret
    \item Issue-adjusted ideal points represent votes better and
      provide a window into issue preferences
  \end{itemize}
}
...
%    \end{figure}
  ...
  \small
  \begin{itemize}
  \item Develop a probabilistic model to capture the influence of documents.
...
\frame{
  ...
  \begin{center}
%  \begin{tabular}{cc}
   \includegraphics[width=0.92\textwidth]{figs/topics.pdf}
%  \vspace{60pt} &
%    \hspace{-40pt} \includegraphics[width=0.55\textwidth]{figs/dtm.pdf}
%  \\
%  \end{tabular}
...
\frame {
  \frametitle{A model of influence}
  We encode several assumptions into a statistical model.
  \begin{itemize}
  \item Assumption 1: Documents at time $t$ come from LDA:
    \begin{figure}
      \includegraphics[width=0.3\textwidth]{figs/dtm.pdf}
    \end{figure}
  \item Assumption 2: Topics $\beta_k$ evolve over time: 
    \begin{align*}
      \beta_{k,t} = \beta_{k,t-1} + \varepsilon_1 
    \end{align*}
  \item Assumption 3: Topics don't drift randomly:
    \begin{align*} \beta_{k,t} = \beta_{k,t-1} + \sum_{d=1}^D \sum_{W=1}^{N_d} \mbox{Influence}(\textcolor{blue}{I_d}, w_{nd}) + \varepsilon \end{align*}
  \end{itemize}
}
...
%    \begin{itemize}
%   \item Words are generated based on Latent Dirichlet Allocation.
% % :
% %     \begin{align*}
% %       p(\bm W, \bm Z, \beta, \bm \theta) = \prod_{k=1}^K p(\beta_k) \times \prod_{d=1}^D p( \theta_d) \prod_{n=1}^{N_d} p(z_{nd} | \theta_d) p(w_{nd} | z_{nd}, \beta)
% %     \end{align*}
%     % \textcolor{white}{\item Topics drift in a Mawrkov chain:
%     %  \begin{align*} \beta_t \sim \mathcal{N}(\beta_{t-1}, \sigma^2) \end{align*}}
%   \end{itemize}
%   \begin{figure}
%     \hspace{-40pt} \includegraphics[height=0.45\textwidth]{figs/dtm.pdf}
%   \end{figure}
%    % Assumes each document has a weight which affects topic drift...\newline
% %     $l_{d,k} \sim \mathcal{N}(0, \sigma_l^2)$ \newline
% %\vspace{-0.43in}
...
%   \item Documents are generated from Latent Dirichlet Allocation:
%     \begin{align*}
%       D_t \sim \mbox{LDA}( \alpha_t, \beta_t )
%     \end{align*}
%     % \textcolor{white}{\item Topics drift in a Markov chain:
%     %  \begin{align*} \beta_t \sim \mathcal{N}(\beta_{t-1}, \sigma^2) \end{align*}}
%   \end{itemize}
%   \begin{figure}
%     \hspace{-40pt} \includegraphics[height=0.45\textwidth]{figs/dtm_1.pdf}
%   \end{figure}
%    % Assumes each document has a weight which affects topic drift...\newline
% %     $l_{d,k} \sim \mathcal{N}(0, \sigma_l^2)$ \newline
% %\vspace{-0.43in}
...
%   \end{itemize}
%   \begin{figure}
%     \hspace{-40pt} \includegraphics[height=0.45\textwidth]{figs/dtm_2.pdf}
%   \end{figure}
%    % Assumes each document has a weight which affects topic drift...\newline
% %     $l_{d,k} \sim \mathcal{N}(0, \sigma_l^2)$ \newline
% %\vspace{-0.43in}
...
%     \end{align*}
%    \item Topics drift in a Markov chain:
%      \begin{align*} \beta_t = \beta_{t-1} + \varepsilon \end{align*}
%   \end{itemize}
%   \begin{figure}
%     \hspace{-40pt} \includegraphics[height=0.45\textwidth]{figs/dtm_3.pdf}
%   \end{figure}
%    % Assumes each document has a weight which affects topic drift...\newline
% %     $l_{d,k} \sim \mathcal{N}(0, \sigma_l^2)$ \newline
% %\vspace{-0.43in}
...
%    \begin{itemize}
%   \item Documents are generated from Latent Dirichlet Allocation:
%   \begin{align*} D_t \sim \mbox{LDA}(\alpha_t, \beta_t) \end{align*}
%    \item Topics drift in a Markov chain:
%      \begin{align*} \beta_t = \beta_{t-1} \textcolor{red}{ + \mbox{Infl}(z, w)} + \varepsilon \end{align*}
%   %   \small \textcolor{white}{\cite{blei:2006}} \\
%   \end{itemize}
%   \begin{figure}
%     \hspace{-40pt} \includegraphics[height=0.45\textwidth]{figs/dtm_4.pdf}
%   \end{figure}
%    % Assumes each document has a weight which affects topic drift...\newline
% %     $l_{d,k} \sim \mathcal{N}(0, \sigma_l^2)$ \newline
% % \vspace{-0.43in}
...
\frame {
  ...
  \begin{itemize}
    \item We observe only the words $\bm W$.
      \vspace{20pt}
    \item We want to find the posterior $p(\bm I, \theta, \bm Z, \beta | \bm W)$.
      \vspace{20pt}
    \item We use variational methods. \tiny \textcolor{gray}{\citep{jordan:1999}} \normalsize
  \end{itemize}
  % \begin{centering}
  %   \begin{figure}
...
\frame{
  ...
  \item We analyzed three corpora with the DIM.
    \begin{itemize}
    \item The \emph{ACL Anthology}: 7,561 documents
    \item \emph{Nature}: 34,454 documents
    \item \emph{PNAS}: 11,855 documents
    %\item 40K documents
    \end{itemize}
  \vspace{20pt}
...
\frame{
  End of influential docs section.
}
...
\frame{
  ...
  Markov step:
  $\beta_{t,k,w} = \beta_{t-1,k,w} \textcolor{red}{ + \sum_{d=1}^D \sum_{W=1}^{N_d} \mbox{Influence}(\textcolor{blue}{I}, w_{nd})} + \varepsilon $
  \vspace{30pt}
  \newline
  We developed the ``influence function'' to have certain characteristics:
  \begin{itemize}
    \item Assumption 4: More recent documents have greater influence.
      \begin{itemize}
        \item Progress is sequential
        \item Empirically, we see that research has a short half-life
        \end{itemize}
    \item Assumption 5: A document only influences relevant topics.
      \begin{itemize}
...
\frame{
  \frametitle{Results for the }
  \includegraphics[width=0.8\textwidth]{figs/fraction_docs_vs_fraction_citations.pdf}
}
...
\frame {
  ...
  For time $t = 1, \ldots, T$:
		  ...
		  \mathcal{N}(\beta_{t-1,k} + \mbox{Influence}(t,k), \sigma^2 I)$
               \item For each document $d_t$:
               \begin{itemize}
   	         \item Generate document $d_t$ using traditional LDA with parameters
	          $\alpha_t$ and $\beta_{t}$.
