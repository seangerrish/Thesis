...
\mytab{notation} provides many of the variables used in this work.
...
\myfig{data_analysis_pipeline}.\footnote{This pipeline is very much
...
\item \textcolor{blue}{\textbf{Modeling assumptions.}} Once we have
  ...
  Chapters 3-6 and \myapp{stochastic_variational_optimization}.
...
\myeq{gm_definition} for all of its corresponding graphical
...
\myfig{bagofwords_lda_gm} (right), for example, one corresponding
...
\end{align*}
  ...
  \myfig{bagofwords_lda_gm} (left).  The bag-of-words assumption in
...
Formally, we can represent this using a \emph{generative process} for
...
\end{align}
We illustrate the graphical model for LDA in \myfig{bagofwords_lda_gm}.  Given the graphical model, we can immediately write the joint distribution of a collection of $D$ documents as
\begin{align}
...
\mytab{example_lda_topics}.  Note that some of these ``words'' are
...
This conditional distribution is impossible to compute efficiently because of the
...
\mysec{variational_inference} we will see details on ways to get around
...
graphically in \myfig{irt_gm}.

In this model, $b_d$ serves as an intercept describing whether the
...
foreign relations in \mychap{foreign_relations}.
...
We will use these time-series abstractions in modeling time-series
...
We have not yet described a limitation of variational inference: with
...
analysis pipeline in \mysec{data_analysis_pipeline}).  The goal of
