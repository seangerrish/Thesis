...
variational inference here and will use use these methods as we
develop models in later chapters.
...
Variational methods posit a simplified family of distributions and
select the member $q_\theta$ of this family that is closest in
KL-divergence to the true posterior $p(x | y)$:
\begin{align}
...
\begin{figure}
  ...
  \caption{Illustration of variational inference.  Practitioners
    define a variational family (illustrated in yellow) and find the
    member of that familt $q_{\hat \eta}(x)$ which is closest (by KL
