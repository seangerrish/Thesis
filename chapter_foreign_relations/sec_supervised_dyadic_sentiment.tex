...
poor because it provides no natural way to model countries that are
frequently in conflicts.  If the inner-product model is endowed with intercepts
...
evaluated the models with up to 9 dimensions, and they TODO(sgerrish),
modeling articles' sentiment at 0.32.

\paragraph{The benefit in adding a time-series assumption.}
We can sometimes improve the predictive power of this model by adding a time
dimension, allowing $\bar x_c$ to drift over time for each country
$c$.  We illustrate these results in \myfig{fr_supervised_performance}.
...
labels.  The better performance of these models for one set of labels
suggests that the formal relationships of these countries -- as
determined by expert labels -- is indeed changing over time; while the
lay relationships between these countries -- as determined by lay
interpretations of countries' relationships -- remains more static
over time.
