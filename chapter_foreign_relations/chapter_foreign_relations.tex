...
first estimated on a training set using Amazon Mechanical Turk (AMT or
Mechanical Turk).

Mechanical Turk is a crowdsourcing platform which provides a
``requestor'' access to thousands of ``workers'' who perform simple
tasks over the Internet.  Common tasks include manually labeling
training examples for machine learning tasks.  As the requestor, we
provided workers with the instruction to label paragraphs to perform these tasks
then attached to all articles by predicting their sentiment.
...
rate the sentiment between two nations mentioned in a paragraph in the
range -5 = "mortal enemies'', $\ldots, 5 = $''very good
relationship''.  With these training examples, we fit the coefficients
$\beta$ of the text regression discussed in
Section~\ref{section:model}.  This coefficient was then fixed in the
joint model in Figure~\ref{figure:gm} to allow us to learn sentiment
from paragraphs' words.  We trained and fit the model with the
following procedure:
...
\subsection{Expert labels: Correlates of War}

We also apply this model with an alternative
...
\end{table}
 ...
 \begin{table}[t]
   ...
   \begin{tabular}{|c|c|c|c|}
     \hline
     Model & Details & Mean Squared Error & Mean Absolute Error \\
     \hline
...
An alternative model of the relationships between countries is 
