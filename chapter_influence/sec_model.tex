...
Our model is based on the dynamic topic model (DTM)~\cite{blei:2006},
...
according to \myeq{logistic-normal}.  For each time point, the words
of its articles are drawn independently from \myeq{softmax}.  One can
...
have a high posterior influence score.  We show in \mysec{results}
...
$w$.  The graphical model is illustrated \myfig{gm}.
...
See~\cite{osareh:1996} for a review.  Much work in this area uses the % citet
link structure of citation networks to extract higher level structure.
\cite{borner:2003} for example, have used author and citation % citet
...
\cite{mann:2006} use topic models and citations to map topics over % citet
...
Two important pieces of recent research have similar
goals. \cite{leskovec:2009} describe a framework for tracking the  % citet
spread of memes, or ideas, in document collections, and investigate
the direction in which ideas tend to percolate.
\cite{shaparenko:2007} describe a measure of influence by modeling   % citet
