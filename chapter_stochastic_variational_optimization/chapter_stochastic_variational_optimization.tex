...
In this paper, we present an alternative algorithm for variational
...
a gradient \citep{carbonetto:2009, graves:2011}.  Carbonetto et
...
distributions in \mysec{so_gaussian}, \mysec{complicated_posteriors} and
...
experiments section
\citep{wei:1990}. \myfig{comparision_order_sampling} (top
...
\small
\begin{algorithm}[H]
  % For numbers, add [1] to this.
    \begin{algorithmic}
%      \caption*{\hspace{50pt} First-order SVO}
\small
%    \setlength{\topsep}{1pt}
...
\begin{minipage}{2.9in} \small
  \begin{algorithm}[H]
    \begin{algorithmic}
 %\caption*{\hspace{50pt} Second-order SVO}
 \small
%    \setlength{\topsep}{1pt}
    ...
    \STATE Draw samples $x_{n1}, \ldots, x_{nM} \sim q_{\theta_{n - 1}}$ using quasi-Monte Carlo sampling.
    \STATE Compute $\partl{\log q_{\theta_n}(x_{nm})}{\theta} \bigr|_{\theta_{n-1}}$.
    \STATE Compute $\partll{\log q_{\theta_n}(x_{nm})}{\theta} \bigr|_{\theta_{n-1}}$. % for $m = 1 \ldots, M$.
...
\myfig{second_order_algorithm} (right), and it can be used instead
of the first-order algorithm
(\myfig{first_order_algorithm} (left)), just as in stochastic
...
In \mysec{so_algorithm_derivation}, we noted that $C$ can be
...
\mysec{so_gaussian}, Bayesian logistic regression,
...
\mysec{so_gaussian} and compare our estimates of
...
\mysec{so_gaussian}, tracking an exponential
